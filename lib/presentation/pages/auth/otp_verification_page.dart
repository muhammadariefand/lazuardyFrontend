...
class _OtpVerificationPageState extends State<OtpVerificationPage> {
  ...
  void _onVerify(String email, RoleEnum? role) {
    if (!_isOtpComplete) {
    ...
    // Panggil Cubit
    switch (widget.otpPurpose) {
      case 'register':
        switch (role) {
          case RoleEnum.student:
            context.read<AuthCubit>().verifyOtpEmail(
              email: email,
              otp: _otpValue,
            );
            break;
          ...
          default:
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Tujuan OTP (role) tidak dikenali'),
                backgroundColor: Colors.red,
              ),
            );
        }
        break;
      // Tambahkan case lain jika ada purpose berbeda
      default:
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Tujuan OTP (purpose) tidak dikenali'),
            backgroundColor: Colors.red,
          ),
        );
    }
  ...
  Widget build(BuildContext context) {
    ...
    final RoleEnum? roleArg = args?['role'] as RoleEnum? ?? widget.role;
            ...
            if (state is VerifyOtpRegisterEmailSuccess) {
              ...
              switch (widget.otpPurpose) {
                case 'register':
                  switch (roleArg) {
                    ...
                    default:
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Role tidak dikenali untuk navigasi'),
                          backgroundColor: Colors.red,
                        ),
                      );
                  }
                ...
                case 'forgot_password':
                  // Tujuannya sama, jadi bisa digabung
                  break;
                default:
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text(
                        'Kesalahan Frontend belum diatur untuk tujuan OTP ini',
                      ),
                      backgroundColor: Colors.red,
                    ),
                  );
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: List.generate(
                      ...
                      (i) => _OtpBox(
                        ...
                        enabled:
                            state is! AuthLoading, // Matikan input saat loading
                      ),
                    ),
                  ),
                  ...
                  PrimaryButton(
                    label: 'Verifikasi',
                    onPressed: () => _onVerify(emailArg, roleArg),
                    isLoading:
                        state is AuthLoading, // Loading otomatis dari Cubit
                  ),
                  ...
                  Center(
                    child: Row(
                      ...
                      children: [
                        ...
                        GestureDetector(
                          onTap: (state is! AuthLoading && _timerExpired)
                              ? _onResend
                              : null,
                          child: Text(
                            'Kirim Lagi',
